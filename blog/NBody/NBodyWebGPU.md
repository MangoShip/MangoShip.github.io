...
3. Create and initialize a Float32Array that stores the properties of all particles. (position and velocity)
...
There are four variables that I will need to initialize. I went through this [link](https://www.w3.org/TR/webgpu/) to learn about them.
...
4. Format: Specifies order of components, bits per component, and data type for the component. It will be used for context configuration. `bgra8unorm` means `blue, green, red, and alpha` `8 bits per components` `unsigned normalized data type`

## 2. Render and Compute Pipeline

In my project, I will be using `.wgsl` files to create functions that can be used by render and compute pipelines. Following is my code for creating a render pipeline:
#### mainWebGPU.ts
```
...
The use of render pipeline is to determine if the shape and color of an object that I want to create. Since I will be creating particles, which is the most simple shape, my code ended up being pretty straightforward. The variable `renderPipeline` creates a render pipeline using the information from `spriteWGSL`. It will use the information of `vertex` (size), and the information of `fragment` (color) to render objects to canvas. 
...
This was also something that I didn't have to worry about, since particles have fixed size. If you are wondering how changing the size of objects can be done here is an example of determining the size of `triangle-list` topology: [Source](https://austin-eng.com/webgpu-samples/samples/computeBoids#main.ts)
#### main.ts
```
...
Let's say you want to make the triangle a little wider, then you would want to replace x values of the first (e.g. -0.01 -> -0.02) and the second vertex. (e.g. 0.01 -> 0.02)
